...
- **Formato visual del Excel de ML** (`analyzer.py` → `_formatear_hoja`, corre al final de cada hoja en `fill_blank_template`): oculta las filas de apoyo de ML entre el header y la fila de ejemplo (que además se borra), letra tamaño 8. Columnas A y B fijas (`freeze_panes = "C1"`), con ancho ajustado al contenido de los productos (nunca al texto del header ni a filas ocultas); el resto de columnas van a ancho fijo 10 con `wrap_text` para no cortar el contenido. Cada variante lleva su fila en blanco separadora (la plantilla trae valores fantasma ahí); la que además cierra un producto (última variante antes de que empiece el siguiente producto) se resalta en amarillo para que la frontera entre productos se note a simple vista.
